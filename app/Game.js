...
class Game {
    constructor({ player, playerPoints, playerPoints_two, playerPoints_two_wrap, dealerPoints, table, hitButton, hit_bet_one, hit_bet_two, standButton, messageBox, playAgainButton, fundsButton, fundsAgainButton, insurance, bankMessage, vol, bet, bet_two, bet_two_wrap, funds, betAsk, passBet, betDisplay, betSet, insuranceBox, insFund, insCancel, insuranceWrap, insuranceAmo, isInsuranceSet, double, split, is_split_set, bet_two_result, info, insurance_info, split_info, isInsuranceAvailable, is_split_available, is_bet_two_won, bet_two_win_val, result_info }) {
    ...
    run() {
        ...
        this.vol.addEventListener('input', (event) => this.amount.textContent = `${this.vol.value}$`);
    ...
    playBetTwo() {
        ...
        if (this.player.points_bet_two <= 21 && this.player.points_bet_two == this.dealer.points) {
        ...
        if (this.player.points_bet_two == 21) {
            ...
            this.bet_two_win_val = (bank.playerBet_two + (bank.playerBet_two * 0.5)).toFixed(2);
            return;
        }
        ...
        if (this.dealer.points > 21) {
            ...
            this.is_bet_two_won = true;
            this.bet_two_win_val = (bank.playerBet_two + (bank.playerBet_two * 0.25)).toFixed(2);
            return;
        }
    ...
    endTheGame() {
        ...
        if (this.player.points <= 21 && this.player.points == this.dealer.points) {
            if (this.is_split_set == false) {
                ...
                this.result_info = `Bet I Tie\n${this.bet_two_result}\nWin: ${this.is_bet_two_won ? this.bet_two_win_val : 0}$`;
                this.balanceCheck();
            }
        ...
        if (this.dealer.points > 21) {
            ...
            if (this.is_split_set == false) {
                ...
                this.balanceCheck();
                setTimeout(() => { this.balanceCheck() }, 3000);
            }
            // this.updateFunds(bet);
            bet.textContent = `${bank.clearBet()}$`;
            this.funds.textContent = `${(bank.playerMoney).toFixed(2)}$`;
